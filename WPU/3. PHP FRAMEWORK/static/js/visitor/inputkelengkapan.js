...
// End sweetalert Karyawan
var loadFotoTemuanBan = function (event) {
  var output = document.getElementById("temuanBan");
...
var loadFotoTemuanKebocoranRem = function (event) {
  var output = document.getElementById("temuanKebocoranRem");
  output.src = URL.createObjectURL(event.target.files[0]);
  output.onload = function () {
    URL.revokeObjectURL(output.src); // free memory
  };
